...
export const LoginFormCase: React.FC<CaseProps> = ({ variant }) => {
  ...
  if (variant === 'A') {
    // ANTI-PATTERN DESIGN (Side A): Vague, tiny low-contrast inputs, confusing submit button
    return (
      <div className="w-full max-w-sm mx-auto text-slate-500 font-sans py-2">
        <h3 className="text-xs uppercase tracking-widest text-slate-400 font-bold mb-5 text-center font-mono">
          Authentication
        </h3>
        <div className="space-y-3.5">
          {/* No Labels, only vague placeholders */}
          <div>
            <input
              type="text"
              ...
              className="w-full px-3.5 py-2.5 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-500 placeholder-slate-300 focus:outline-none"
            />
          </div>
          ...
          {/* Low contrast, confusing CTA button */}
          <button
            type="button"
            className="w-full py-2.5 bg-slate-200 text-slate-500 rounded-xl text-xs font-semibold cursor-pointer transition border border-slate-300/60"
          >
            Submit
          </button>

          <div className="text-xs text-slate-400 text-center italic mt-1 font-mono">
            * Data input error code #401
          </div>
        </div>
      </div>
    );
  ...
  // BEST PRACTICE DESIGN (Side B): Clear visual hierarchy, accessible inputs, WCAG AAA compliant
  return (
    <div className="w-full max-w-md mx-auto text-slate-900 font-sans py-2">
      ...
      <div className="space-y-4">
        {/* Clear Top Label + Icon + Accessible Input */}
        <div>
          <label className="block text-xs sm:text-sm font-bold text-slate-800 mb-1.5 flex items-center justify-between font-sans">
          ...
          <div className="relative">
            ...
            <input
              ...
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-300 rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-blue-500/15 focus:border-[#0560FD] focus:bg-white transition font-sans"
            />
          </div>
        </div>
        ...
        <div>
          ...
          <div className="relative">
            ...
            <input
              ...
              value={password}
              onChange={(e) => setPassword(e.target.value)}
              className="w-full pl-10 pr-11 py-3 bg-slate-50 border border-slate-300 rounded-xl text-sm text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-blue-500/15 focus:border-[#0560FD] focus:bg-white transition font-sans"
            />
            <button
              type="button"
